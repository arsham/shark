...
local function lsp_organise_imports() --{{{
  ...
  for _, client in ipairs(get_clients()) do
    ...
    local resp = vim.lsp.buf_request_sync(0, "textDocument/codeAction", params, timeout)
    for _, res in pairs(resp or {}) do
      for _, r in pairs(res.result or {}) do
        if r.edit then
          -- Apply the workspace edit using the correct offset encoding.
          vim.lsp.util.apply_workspace_edit(r.edit, client.offset_encoding)
        else
          -- Execute the command if there is no edit.
